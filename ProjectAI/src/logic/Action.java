		...
		canvas.addPaintListener(new PaintListener() {
			...
			public void paintControl(PaintEvent e) {
				...
				if(defaultValuePrecLenght) {
					lengthPrec=getLenght(prec)*avergWidth;
				}
				...
				if(defaultValueEffLenght) {
					lengthEff=getLenght(effect)*avergWidth;
				}
					...
					public void handleEvent(Event event) {
						IDialog dialog = new IDialog(boxSize.getParent().getShell()) {
							...
							public void createContent() {
								this.getLabel().setText("set the Box-size: " + getName());
								this.getLabel().pack();
				...
				precSize.addListener(SWT.Selection, new Listener() {
					...
					public void handleEvent(Event event) {
						IDialog dialog = new IDialog(boxSize.getParent().getShell()) {
							...
							@Override
							public void createContent() {
								this.getLabel().setText("set the PrecLine-size of the action: " + getName());
								this.getLabel().pack();
								Composite c = getComposite();
								c.setLayout(new GridLayout(2, false));
				...
				effSize.addListener(SWT.Selection, new Listener() {
					...
					public void handleEvent(Event event) {
						IDialog dialog = new IDialog(boxSize.getParent().getShell()) {
							...
							@Override
							public void createContent() {
								this.getLabel().setText("set the EffectLine-size of the action: " + getName());
								this.getLabel().pack();
								Composite c = getComposite();
								c.setLayout(new GridLayout(2, false));
